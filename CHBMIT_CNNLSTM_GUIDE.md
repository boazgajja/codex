...
## Kaggle (RAM/disk limited) strategy

Your plan is correct: **do not load full CHB-MIT at once**.

### 1) Install packages in notebook

```bash
!pip install -q boto3 mne scikit-learn torch
```

If you want TensorFlow version later:

```bash
!pip install -q tensorflow
```

### 2) Use incremental mode (stream small file chunks)

This mode trains with a few EDF files at a time, then frees RAM.

```bash
!python chbmit_cnn_lstm_pipeline.py \
  --download \
  --download_dir /kaggle/temp/chbmit \
  --incremental \
  --chunk_edf_files 3 \
  --window_sec 4 --stride_sec 2 \
  --target_fs 128 \
  ...
  --batch_size 32 \
  --use_focal \
  --checkpoint_path /kaggle/working/cnn_lstm_chbmit_ckpt.pt
```

### 3) Free disk after training
...
```bash
!rm -rf /kaggle/temp/chbmit
```

### 4) Important Kaggle path rule

- Use `/kaggle/temp` for temporary downloaded data.
- Save final model/results into `/kaggle/working` (this gets persisted in notebook output).
